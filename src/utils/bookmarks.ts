...
// 生成书签id，根据时间戳生成一个纯数字递增随机id
function generateBookmarkId(timestampInput?: number) {
  const timestamp = timestampInput || Date.now();
  const random = Math.floor(Math.random() * 1000).toString().padStart(3, '0'); // 生成0-999的随机数，并补全为3位
  return `${timestamp}${random}`; // 拼接时间戳和随机数
}

export { generateBookmarkId };